...
class Goldberg:
    ...
    def __print_residuals(self, index):
        ...
        #              output="cf/cf_residuals" + str(index) + ".pdf", vertex_text=labels, edge_text=self.res)
        print("Print graph "+str(index))
        self.i += 1
    ...
    def send_flow(self, source, target, value):
        ...
        self.excess[target] += value
        self.excess[source] -= value
    ...
    def init(self, source):
        ...
        for edge in self.graph.edges():
            self.res[edge] = self.capacity[edge] - self.flow[edge]
    ...
    def get_active_vertex(self):
        for vertex in self.graph.vertices():
            if self.excess[vertex] > 0 and vertex != self.source and vertex != self.sink:
                return vertex
        return False
