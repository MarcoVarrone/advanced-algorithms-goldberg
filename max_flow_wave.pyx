...
cdef class GoldbergWave:
    ...
    cpdef get_max_flow(self, source, sink):
        self.source = source
        self.sink = sink
        self.init(source)
    ...
    cdef push(self, vertex):
        success = False
        #print("Chosen vertex:" + str(vertex))
        for edge in list(vertex.out_edges()):
            #print("Edge:"+str(edge)+" excess "+str(self.excess[vertex]))
    ...
    cdef init(self, source):
        ...
        for edge in list(source.out_edges()):
            ...
            #self.actives.insert(0, edge.target())
    ...
    cdef set_excess(self, vertex, int value):
        ...
        if value > 0:
            ...
            if vertex in self.actives:
                self.actives.remove(vertex)